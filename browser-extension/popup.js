...
function drawDeviceClock() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const innerR = isMajor ? radius - 10 : radius - 7;
    ctx.beginPath();
  ...
  drawHand(ctx, ((hours + minutes / 60) * Math.PI) / 6, radius * 0.5, 3, "#2c3e50");
  drawHand(ctx, ((minutes + seconds / 60) * Math.PI) / 30, radius * 0.72, 2, "#2c3e50");
  drawHand(ctx, (seconds * Math.PI) / 30, radius * 0.8, 1, "#e74c3c");
...
function updateCityTime() {
  ...
  cityTimeEl.textContent = `${cityName}: ${hour12}:${pad(minutes)}:${pad(seconds)} ${period}`;
}
